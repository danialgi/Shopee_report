...
st.set_page_config(page_title="Shopee Report", page_icon="🚚", layout="wide")

st.title("🚚 Genuine Inside (M) Sdn. Bhd. - SHOPEE Report📋")
st.markdown("##")

st.header("Shopee File Upload")
...
df_tiktok_new=df_tiktok[['Order Creation Date','Order Paid Time','Estimated Ship Out Date','Order ID','Shipping Option','Tracking Number*']].copy()
df_tiktok_new['tarikh'] = pd.to_datetime(df_tiktok['Order Creation Date']).dt.date
...
tiktok_filtered=tiktok_filtered.sort_values(by='Tracking Number*', ascending=True)
...
filtered_df_wms_new = df_wms_new[df_wms_new['Marketplace'] == 'TikTok']
...
st.markdown("#")
st.write("")
df_compare=pd.concat([df_wms_new['Tracking No.'], tiktok_filtered['Tracking Number*']])
...
tiktok_filtered['WMS'] = np.where(tiktok_filtered['Tracking Number*'].isin(df_wms_new['Tracking No.']), True, False)
tiktok_filtered.sort_values(['WMS', 'Tracking Number*'], ascending=[False, True], inplace=True)
tiktok_filtered.reset_index(inplace=True)
tiktok_filtered = tiktok_filtered.drop('index', axis=1)
...
st.download_button(
    ...
    data=excel_file,
    file_name=f"Shopee_Report_{today_date}.xlsx",
